...
import { accountCookieName } from '../utils/constants'
...
export const useUser = () => {
  ...
  const [cookies, setCookie, removeCookie] = useCookies([accountCookieName])
  ...
  useEffect(() => {
    if (!user) {
      removeCookie(accountCookieName)
      return
    }
    setCookie(accountCookieName, user)
